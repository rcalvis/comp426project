...
app.get("/search", async(req, res) => {
    ...
    try {
        const response = await axios.get(`https://imdb.iamidiotareyoutoo.com/search?q=${searchQuery}`);
...
app.delete("/delete-movie", (req, res) => {
    ...
    req.session.user.list = req.session.user.list.filter(movie => movie.id != movieID);
    res.status(200).json({message: "Movie removed from user list", list: req.session.user.list});
})
